...
const TokenSaleCard = ({
  ...
  // Logic for validation helpers inside the UI
  ...
  return (
    <div className="bg-gray-800 p-8 rounded-xl shadow-2xl w-full border border-gray-700 relative overflow-hidden">
        ...
        <div className="flex items-center relative">
          <input
            ...
            className={`w-full bg-gray-900 border rounded-l-lg p-3 text-xl focus:outline-none transition-colors duration-200 ${
              isInsufficientBalance ? "border-red-500 text-red-300" : "border-gray-600 focus:border-blue-500 text-white"
            }`}
          />
          <div className="bg-gray-700 border border-gray-600 border-l-0 rounded-r-lg p-3 px-4 font-bold text-gray-300 w-24 text-center">
            {currency}
          </div>
        </div>
      ...
      <button
        onClick={handleBuy}
        disabled={isLoading || !account || !isCorrectNetwork || isInsufficientBalance || isInvalidAmount || isSalePaused}
        className={`w-full py-4 rounded-lg font-bold text-lg transition-all duration-200 shadow-lg ${
          (isLoading || isInsufficientBalance || isInvalidAmount || !account || isSalePaused)
