...
describe("Invariant: User Cap", function () {
  ...
  it("User contribution must never exceed MAX_CONTRIBUTION", async function () {

    const cap = await crowdsale.MAX_CONTRIBUTION();

    const contribution = await crowdsale.userTotalUSD(user.address);

    expect(contribution).to.be.lte(cap);
