    ...
    const clusters = [
        {
            ...
            color: "#A855F7",
            items: [
                { label: "Public Sale (ICO)", subLabel: "In Phases (Event Based Burn)", value: "30.0", amount: "15", unit: "B", color: "#A855F7" },
        ...
        },
        {
            ...
            items: [
                ...
                { label: "Team & Strategic Advisors", subLabel: "Cliff & Vesting Schedule", value: "10.0", amount: "5", unit: "B", color: "#60A5FA" },
            ]
        },
        {
            id: 2, // Bottom-Left
        ...
        },
        {
            ...
            color: "#EAB308",
            items: [
                { label: "Company Inventory", subLabel: "Platform Reserve\n25% Burn & 75% Recycle", value: "30.0", amount: "15", unit: "B", color: "#EAB308" },
            ]
        }
    ];

    // The "Effective" index used for all visual states
                ...
                <div className="relative w-full max-w-6xl mx-auto flex flex-col items-center">
                    ...
                    <div className="lg:hidden w-full space-y-4 px-4 pb-12">
                        <div className="w-full py-8 text-center flex flex-col items-center">
                            ...
                            <div className="px-6 py-3 border border-white/10 bg-white/5 rounded-full flex flex-col items-center gap-1 shadow-xl">
                                ...
                                <div className="text-[10px] font-black text-amber-400 tracking-[0.3em] uppercase mt-1">Burn & Recycle</div>
                            </div>
                        </div>
                    ...
                    <div className="hidden lg:block relative w-full h-[650px] max-w-5xl mx-auto">
                        ...
                        <div className="absolute inset-0">
                            {/* Top-Left */}
                            <div
                                className="absolute top-[5%] left-0 z-40"
                                onMouseEnter={() => { setIsHovered(true); setHoveredIndex(0); }}
                                onMouseLeave={() => { setIsHovered(false); setHoveredIndex(null); }}
                            >
                                <ClusterCard cluster={clusters[0]} isActive={displayIndex === 0} />
                            </div>
                            {/* Top-Right */}
                            <div
                                className="absolute top-[5%] right-0 z-40"
                                onMouseEnter={() => { setIsHovered(true); setHoveredIndex(1); }}
                                onMouseLeave={() => { setIsHovered(false); setHoveredIndex(null); }}
                            >
                                <ClusterCard cluster={clusters[1]} isActive={displayIndex === 1} />
                            </div>
                            {/* Bottom-Left */}
                            <div
                                ...
                                onMouseLeave={() => { setIsHovered(false); setHoveredIndex(null); }}
                            >
                                <ClusterCard cluster={clusters[2]} isActive={displayIndex === 2} />
                            </div>
                            {/* Bottom-Right */}
                            <div
                                className="absolute bottom-[5%] right-0 z-40"
                                onMouseEnter={() => { setIsHovered(true); setHoveredIndex(3); }}
                                onMouseLeave={() => { setIsHovered(false); setHoveredIndex(null); }}
                            >
                                <ClusterCard cluster={clusters[3]} isActive={displayIndex === 3} />
                            </div>
                        </div>
                        ...
                        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                            <div className="relative w-80 h-80 pointer-events-auto">
                                ...
                                <motion.div
                                    ...
                                    animate={{ rotate: rotationMap[displayIndex] }}
                                ...
                                >
                                    <motion.div
                                        ...
                                        animate={{ backgroundColor: clusters[displayIndex].color, boxShadow: `0 0 45px ${clusters[displayIndex].color}` }}
                                ...
                                <motion.div
                                    ...
                                    animate={{ backgroundColor: clusters[displayIndex].color }}
                                ...
                                <div className="relative w-full h-full rounded-full border-[12px] border-white/5 bg-[#0a0a0a] flex flex-col items-center justify-center shadow-2xl">
                                    ...
                                    <div className="text-center relative z-20">
                                        ...
                                        <div className="text-[10px] font-black text-amber-400 tracking-[0.3em] my-2 uppercase">Burn & Recycle</div>
                                        <div className="text-blue-400 font-bold text-xs tracking-[0.2em]">{TOKEN_TICKER}</div>
                                    </div>
                                    <motion.div
                                        className="absolute inset-0 rounded-full border opacity-15"
                                        animate={{ scale: [1, 1.3], opacity: [0.6, 0], borderColor: clusters[displayIndex].color }}
...
const ClusterCard = ({ cluster, isActive, forceActive = false }) => {
    ...
    return (
        <motion.div
            ...
            className={`w-full lg:w-[300px] border rounded-2xl p-6 backdrop-blur-3xl shadow-2xl flex flex-col ${!activeState && 'grayscale'
                ...
                {cluster.items.map((item, idx) => (
                    <div key={idx} className="relative">
                        ...
                        <div className="flex justify-between items-end">
                            <span className="text-[10px] text-gray-600 uppercase tracking-widest font-bold whitespace-pre-line leading-relaxed">{item.subLabel}</span>
                            {item.amount && (
