...
const LegalModal: React.FC<LegalModalProps> = ({ isOpen, onClose, title, content }) => {
  ...
  return (
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md transition-opacity duration-300">
      ...
      {/* MODAL WINDOW */}
      <div 
        className="bg-black border border-white/10 w-full max-w-4xl h-[80vh] rounded-2xl shadow-2xl flex flex-col relative animate-fade-in-up"
        onClick={(e) => e.stopPropagation()} // Prevent clicking inside from closing
      >
        
        {/* HEADER */}
        <div className="flex items-center justify-between p-6 border-b border-white/5 bg-black rounded-t-2xl sticky top-0 z-10">
        ...
        {/* SCROLLABLE CONTENT */}
        <div className="flex-1 overflow-y-auto p-6 md:p-10 text-gray-300 leading-relaxed text-sm md:text-base space-y-4 custom-scrollbar bg-black">
            {content.split('\n').map((line, index) => (
        ...
        {/* FOOTER ACTION */}
        <div className="p-6 border-t border-white/5 bg-black rounded-b-2xl flex justify-end">
            <button 
                onClick={onClose}
